...
""".format(
        ".".join(str(n) for n in PYTHON_MIN_VERSION),
...
def file_walk_relative(root):
    ...
    """
    return [str(p.relative_to(Path("lib") / "cartopy")) for p in root.glob("**/*")]
...
try:
    ...
    geos_version = tuple(
        int(v) for v in geos_version.split(b".") if "dev" not in str(v)
    )
    ...
    if geos_version < GEOS_MIN_VERSION:
        print(
            "GEOS version %s is installed, but cartopy requires at least "
            "version %s."
            % (
                ".".join(str(v) for v in geos_version),
                ".".join(str(v) for v in GEOS_MIN_VERSION),
            ),
            file=sys.stderr,
        )
...
if not sys.platform.startswith("win"):
    extra_extension_args["runtime_library_dirs"].append(get_config_var("LIBDIR"))
...
if cython_coverage_enabled:
    extra_extension_args["define_macros"].append(("CYTHON_TRACE_NOGIL", "1"))

extensions = [
    Extension(
        "cartopy.trace",
        ["lib/cartopy/trace.pyx"],
        include_dirs=([include_dir, "./lib/cartopy", np.get_include()] + geos_includes),
