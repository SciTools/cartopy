...
class CRS(pyproj.crs.CustomConstructorCRS):
    ...
    def transform_vectors(self, src_proj, x, y, u, v):
        ...
        if self == src_proj:
            # No rotation is possible when the source and target are the
            # same CRS: the Jacobian is exactly the identity everywhere,
            # including at source domain edges where the finite-difference
            # probes below would otherwise be unreliable.
...
class TransverseMercator(Projection):
    ...
    def __init__(self, central_longitude=0.0, central_latitude=0.0,
        ...
        self.threshold = min(a, b) / 637.8137  # Approximately 1e4 for defaults.
