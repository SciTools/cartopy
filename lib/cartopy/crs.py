...
class HEALPix(Projection):
    ...
    def __init__(self, central_longitude=0, globe=None):
        ...
        # Get the width of the map by rotating 90 degrees at the
        # equator which accounts for different ellipsoids.
        w = 2 * self.transform_point(central_longitude + 90, 0,
                                     self.as_geodetic())[0]
        h = w / 2
        self.bounds = [-w, w, -h, h]
        self.threshold = w / 1e4

        self._boundary = shapely.LinearRing(
            [(-w, h / 2), (-3 / 4 * w, h), (-w / 2, h / 2), (-w / 4, h), (0, h / 2),
             (w / 4, h), (w / 2, h / 2), (3 / 4 * w, h), (w, h / 2), (w, -h / 2),
             (3 / 4 * w, -h), (w / 2, -h / 2), (w / 4, -h), (0, -h / 2), (-w / 4, -h),
             (-w / 2, -h / 2), (-3 / 4 * w, -h), (-w, -h / 2), (-w, h / 2)])
