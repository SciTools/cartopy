...
class Projection(CRS, metaclass=ABCMeta):
    ...
    def _attach_lines_to_boundary(self, multi_line_strings, is_ccw):
        ...
        #
        # Build the set of mls_i values that appear exactly once (singletons).
        # A source ring that produced only one boundary segment is the typical
        # signature of a PROJ coordinate-wrapping artefact.
        _seen, _multi = set(), set()
        for _, _, _, mls_i in remaining_segs:
            (_multi if mls_i in _seen else _seen).add(mls_i)
        singleton_mls = _seen - _multi
        remaining_segs = [
            (d_s, d_e, k, mls_i) for d_s, d_e, k, mls_i in remaining_segs
            if not (mls_i in singleton_mls and d_e < d_s
                    and _ring_is_ccw(
                        sgeom.LinearRing(
        ...
        def finish_ring(d_end_local, ring_coords_local, d_fwd_local,
            ...
            corners = arc_corners(d_end_local, d_ring_start_local)
        ...
        while remaining_segs:
            ...
            while True:
                ...
                if not remaining_segs:
                    ...
                    finish_ring(d_end, ring_coords, d_fwd_close, d_ring_start)
                    break
...
def _ring_is_ccw(ring):
    ...
    cancels self-intersecting tail contributions.
    """
