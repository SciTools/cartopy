...
class InterProjectionTransform(mtransforms.Transform):
    ...
    def transform_path_non_affine(self, src_path):
        ...
        geoms = cpatch.path_to_geos(src_path, getattr(self, "force_path_ccw", False))
        ...
        if not transformed_geoms:
            ...
            points, codes = list(
                zip(
                    *[
                        cpatch.path_segments(path, curves=False, simplify=False)
                        for path in paths
                    ]
                )
            )
...
class GeoAxes(matplotlib.axes.Axes):
    ...
    def _wrap_args(self, *args, **kwargs):
        ...
        if not (
            kwargs.get("shading", default_shading) in ("nearest", "auto")
            and len(args) == 3
    ...
    def _wrap_quadmesh(self, collection, **kwargs):
        ...
        #       calls.
        pcolor_data = np.ma.array(np.zeros(C.shape), mask=~mask)
