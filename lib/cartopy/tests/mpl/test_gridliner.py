...
def test_gridliner_visibility_after_draw():
    ...
    fig.canvas.draw()
    bare = np.asarray(fig.canvas.buffer_rgba()).copy()
    bare_title = ax.title.get_position()

    grid = ax.gridlines(draw_labels=True)
    fig.canvas.draw()
    assert not np.array_equal(bare, np.asarray(fig.canvas.buffer_rgba()))
    assert not grid.stale

    grid.set_visible(False)
    fig.canvas.draw()
    np.testing.assert_array_equal(bare, np.asarray(fig.canvas.buffer_rgba()))
