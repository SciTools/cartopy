...
class Test_path_to_shapely:
    def test_empty_polygon(self, legacy):
        p = Path(
            [
                [0, 0], [0, 0], [0, 0], [0, 0],
                [1, 2], [1, 2], [1, 2], [1, 2],
        ...
        if legacy:
            ...
            geoms = cpatch.path_to_shapely(p)
            assert isinstance(geoms, sgeom.MultiPoint)
            assert len(geoms.geoms) == 4

    def test_non_polygon_loop(self, legacy):
    ...
    def test_polygon_with_interior_and_singularity(self, legacy):
    ...
    def test_nested_polygons(self, legacy):
