...
class TestHoles(PolygonTests):
    ...
    def test_inverted_poly_multi_hole(self):
        ...
        inside_pt = sgeom.Point(proj.transform_point(-150, 60, pc))
        assert polygon.contains(inside_pt)

        # point inside square hole should not be contained
        hole_pt = sgeom.Point(proj.transform_point(-25, -25, pc))
        assert not polygon.contains(hole_pt)

        # A point below the southern edge of the polygon shouldn't
        # be contained (could be improperly covered by the entire
        # polygon if not projected correctly).
        south_pt = sgeom.Point(proj.transform_point(0, -85, pc))
        assert not polygon.contains(south_pt)
