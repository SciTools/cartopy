...
def tile_bbox(prj, x, y, z, lat_extent_at_z0=(-85., 85.), bottom_up=True, native=True):
    ...
    result = prj.transform_points(cartopy.crs.PlateCarree(), numpy.array([0., 0]), numpy.array(lat_extent_at_z0))
    y1, y0 = result[:, 1]
    # compute the box height in native coordinates for this zoom level
    box_h = (y1 - y0) / n
    ...
    for z in range(0, 4):
        for i in numpy.arange(0, 2**z):
            for j in xrange(0, 2**z):
                x_lim, y_lim = tile_bbox(prj, i, j, z, bottom_up=False)
                print i, j, z, y_lim
                ax.set_xlim(x_lim)
                ax.set_ylim(y_lim)
