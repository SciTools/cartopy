...
def main():
    ...
    # -82.5, -67.5, ..., 67.5, 82.5
    dlat = 180 // nlat
    lat = np.linspace(-90.0 + dlat / 2.0, 90.0 - dlat / 2.0, nlat)
    # 0, 1, ..., 10, 11, 11, 10, ..., 1, 0
    data = np.concatenate((np.arange(nlon // 2), np.arange(nlon // 2)[::-1]))
    ...
    ax1.set_title('1d longitudes, central longitude=180', fontsize='small')
    ax1.set_global()
    ax1.contourf(lon, lat, data, transform=ccrs.PlateCarree(), cmap='RdBu')
    ax1.coastlines()

    # plot with central longitude 0
    ax2 = fig.add_subplot(2, 2, 2, projection=ccrs.Robinson(central_longitude=0))
    ax2.set_title('1d longitudes, central longitude=0', fontsize='small')
    ax2.set_global()
    ax2.contourf(lon, lat, data, transform=ccrs.PlateCarree(), cmap='RdBu')
    ax2.coastlines()
    ...
    ax3.set_title('Cyclic 1d longitudes, central longitude=180', fontsize='small')
    ax3.set_global()
    ax3.contourf(clon, clat, cdata, transform=ccrs.PlateCarree(), cmap='RdBu')
    ax3.coastlines()
    ...
    ax4 = fig.add_subplot(2, 2, 4, projection=ccrs.Robinson(central_longitude=0))
    ax4.set_title('Cyclic 2d longitudes, central longitude=0', fontsize='small')
    ax4.set_global()
    ax4.contourf(clon2d, clat2d, cdata, transform=ccrs.PlateCarree(), cmap='RdBu')
    ax4.coastlines()
